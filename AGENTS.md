...
Latency/buffering policy: clusters are 60 ms (`--audio-cluster-ms`, was 200) and each `Listen` subscriber gets a bounded 8-slot queue (`--audio-subscriber-queue`) — a subscriber that falls behind has its **oldest** chunks dropped and skips ahead, rather than being disconnected and reconnecting forever. `AudioEvent::Failed` still ends the stream. The client counterparts are pure functions in [web/dist/lib/audio.js](./web/dist/lib/audio.js): `gapSkip()` jumps buffered holes (never wait them out), `trimPlan()` trims behind the playhead, and `liveEdgeSeek()` jumps to the live edge when the buffer's tail runs >10 s ahead (ahead-side data is never `remove()`d — that starves the playhead with a faster-than-real-time producer). Measure with `tests/web/web_latency_test.mjs`. `serve` logs a 5 s `audio stats` line (chunk rate, mean/min cluster ms, xruns, channel stalls, per-subscriber lag drops) — `info`, so it needs `-d` or `RUST_LOG=info`.
...
2. **Browser E2E** — scripted CDP sessions against a *fake scanner* (W5; `tests/fake_scanner.py` on a socat pty pair) or the real hardware (W6). The fake stack is started with `bash tests/ubc125_stack.sh` (idempotent; self-provisions `socat` via `nix-shell`; serves on `127.0.0.1:50051`). The browser must be Edge launched by the **browser-tools skill** (`browser-start.js`, CDP on `:9222`) — the E2E scripts connect to it with `puppeteer-core` imported from that skill's `node_modules`; nothing in the repo installs a browser. Then:
   ...
   node tests/web/web_audio_test.mjs   # audio Play/Stop + throttle + late joiner (24 checks, ~3 min; needs target/debug/ubc125, manages the stack itself; the throttled phase asserts no reconnect *and* a playhead that keeps advancing)
   node tests/web/web_latency_test.mjs   # playhead stall / lead report (starts the fake stack; on the Pi use --no-stack --max-lead-p99 0.6)
   ```
